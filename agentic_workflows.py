...
'''
What we need to create VCT manager chatbot
'''
...
def process_llm_response(response_message, lambda_client, s3):
    ...
    for content_block in response_content_blocks:
        if "toolUse" in content_block:
            ...
            elif tool_use_name == "random_number":
                tool_result_value = generate_random_number(
                    tool_use_block["input"]["min_value"], tool_use_block["input"]["max_value"]
                )
                print(f"Random number result: {tool_result_value}")
                follow_up_content_blocks.append(
                    {
                        "toolResult": {
                            "toolUseId": tool_use_block["toolUseId"],
    ...
    message_list = [
        {
            ...
            "content": [
                {
                    "text": "Generate a random number between 8-18, then calculate its consine"
                }
            ],
        }
    ]
